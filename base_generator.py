...
# Оценка, получаемая каждым из студентов
CURRENT_RATING = 0.8
...
class TestSystem:
    ...
    def __init__(self):
        ...
        #
        self.handlers = (self.rating_handler, \
                         self.tags_handler, \
    ...
    def refresh_rating_file(self):
        "Сохраняет оценки за задания, полученные в текущий запуск."
        r_file = open('rating.dat', 'w')
        vals = sorted(self.students.keys(), key=int)
        for v in vals:
            r_file.write(v + ':')
            for ex in self.students[v].rating.keys():
                r_file.write(str(ex.ID) + '=')
                r_file.write(str(self.students[v].rating[ex]) + ',')
            r_file.write('\n')
        r_file.flush()
        r_file.close()
    #
    ...
    def _load_rating(self):
        ...
        for line in r_file.readlines():
            ...
            #
            if len(first) == 1:
                self.students[ID].rating = {}
            else:
                exs = first[1].split(',')
                for e in exs:
                    parts = e.split('=')
                    if len(parts) > 1:
                        ex_inst = self.exercises[parts[0]]
                        self.students[ID].rating[ex_inst] = float(parts[1])
    #    
    def max_difficulty_handler(self, student):
        ...
        return result
    #
    def rating_handler(self, student):
        '''
        Хэндлер полученных баллов.
        Приоритет в выборе отдаётся темам, по которым получено меньше баллов.
        '''
        result = []
        if len(student.rating) == 0:
            for ex in self.exercises.values():
                result.append((ex, 1.0))
        else:
            for ex in student.rating.keys():
                result.append((ex, 1.0 - student.rating[ex] / ex.reward))
            for ex in self.exercises.values():
                result.append((ex, 0.0))
        #
        result.sort(key=lambda x: x[1], reverse=True)
        return result
    #
            ...
            #
                ...
                if not self._is_arg(arg):
                    ...
                    if answer == 2 or answer == 3:
                        ...
                        self.students[student_id].rating[ex] = \
                            ex.reward * CURRENT_RATING
                        print('Вы получили за это задание ' + \
                            str(self.students[student_id].rating[ex]) + \
                            ' баллов.')
                    self.last_choice = []
                    for i in range(5):
...
if __name__ == '__main__':
    ...
    # print(system.rating_handler(system.students['4'])[:3])
